...
**Version:** 1.2 (2026-08-23: conduct rules only; product/architecture appendix removed, see canonical truth)
**Date:** July 2026  
**Classification:** Engineering — Required Reading  
**Prerequisite:** docs/TRACE-CANONICAL-TRUTH.md (single source of truth) + ../CONTEXT.md (glossary). Historical PRD/Spec/ADRs are archived or superseded - never implement from them.
...
## Where the system facts live

This document is now **conduct and governance rules only**. All as-built architecture,
component statuses, commercial terms, domain registries, security gaps, roadmap, and
superseded-path rulings live in **[docs/TRACE-CANONICAL-TRUTH.md](TRACE-CANONICAL-TRUTH.md)**
(glossary: [CONTEXT.md](../CONTEXT.md); historical ADRs: docs/adr/).

Env-var naming note (corrects former 3.9 examples): the live LLM selector is
`LLM_SERVICE_PROVIDER` (app/services/llm.py); `llm_backend` in app/core/config.py is
vestigial. When in doubt, config code is authoritative.
