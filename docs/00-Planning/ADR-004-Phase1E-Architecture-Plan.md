...
## 12. Future Admin Config — firm_settings (Phase 2)

The `create_llm_service()` factory currently reads provider from the
`LLM_SERVICE_PROVIDER` env var. When the SaaS admin UI is built, the
same factory maps directly to a per-firm configuration table without
architectural rework.

### firm_settings table (Phase 2)

| Column              | Type     | Description                          |
|---------------------|----------|--------------------------------------|
| firm_id             | UUID FK  | Reference to firms table             |
| llm_provider        | VARCHAR  | `azure_openai` \| `deepseek_api`     |
| llm_model           | VARCHAR  | Deployment name override (nullable)  |
| monthly_token_limit | INTEGER  | Hard cap for this firm               |
| tier                | VARCHAR  | `INTAKE` \| `PIPELINE` \| `OPERATIONS` |

### create_llm_service(firm_id) lookup order

1. Check `firm_settings` for `firm_id`
2. Fall back to `LLM_SERVICE_PROVIDER` env var
3. Fail loudly if neither is set — silent fallback is a PHI risk

### Tier gate (admin UI, not factory)

| Tier        | Provider access         | Token cap |
|-------------|------------------------|-----------|
| INTAKE      | azure_openai only      | capped    |
| PIPELINE    | azure_openai           | higher cap|
| OPERATIONS  | any provider           | uncapped  |

The tier gate lives in the admin UI — the factory executes whatever setting
it receives. No code change needed in `create_llm_service()` when this lands.
Only one additional lookup: `SELECT llm_provider FROM firm_settings WHERE
firm_id = $1`, then fall back to env var.
