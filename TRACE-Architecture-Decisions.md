...
| **Object storage** | AWS S3 + SSE-KMS `[IMPLEMENTATION CHOICE]` | **S3 already in use** — `TrueVow_Tenant_SETTLE-Service/app/services/storage/s3_service.py` (boto3, presigned URLs, SSE-AES256, mock mode, `us-west-2`). *(Corrects my earlier "no existing object storage" flag.)* | **S3 via boto3, following SETTLE's pattern, wrapped in the spec's `StorageService` interface — but upgraded to SSE-KMS (customer-managed key) + AWS BAA because TRACE stores PHI.** SETTLE's bucket is non-PHI reports; TRACE needs a separate BAA-covered, KMS-encrypted bucket. |
...
| **Billing LLM** | Azure OpenAI GPT-4o-mini `[LOCKED]`; DeepSeek prohibited | No Azure usage in platform, but Azure OpenAI is a standalone HTTPS API (like Textract) — no hosting coupling. | **Keep Azure OpenAI GPT-4o-mini** (BAA-covered, standalone API). No platform conflict. |
| **OCR** | AWS Textract (cross-cloud OK) | boto3/AWS already present (SETTLE S3). | **Keep Textract** (needs AWS BAA, which the S3 PHI bucket needs anyway). |
...
## D. Still outstanding (unchanged)

- **Updated PRD §12** — still lists **9** open questions, not 12.
- **Amended spec** — Fly.io+Supabase `[IMPLEMENTATION CHOICE]` tables (you elected to issue).

*No Phase 1A code will be written until the sign-off items in C and the outstanding items in D are resolved.*
